 ...
 * @author mike
 */
public class GINSProcessorDebug {
    ...
    protected void updateINSPosition(Instant systemTime, Instant posTimeUTC, Instant velTimeUTC, GINS_T27 t27) {
        ...
        fim.getAltitudeUncertainty().setValue(100);
        ...
        System.out.println("#FIM30;"+posTimeUTC+";"+fim.toString());
        System.out.println("#INS;"+posTimeUTC+";"+t27.getLatitude()+";"+t27.getLongitude()+";"+t27.getAltitude());
    }
        ...
        if(lastT27!=null) {
            if(lastT27.getTimeOfValidity().compareTo(t27.getTimeOfValidity())!=0) {
                if(bestTimeFilter!=null) {
                    Instant utc = bestTimeFilter.calculateUTC(systemTimeOfLast_T27);
    ...
    protected void onT20(GINS_T20 t20) {
        ...
        if(bestTimeFilter!=null) {           
            Instant xutc = bestTimeFilter.calculateUTC(timeOfLast_T20);
        
            Duration dt = Duration.between(utc,xutc);
            System.out.println("#DT_T20;"+dt.toNanos()/1_000_000_000.0);                        
        }

        System.out.println("#UTC_T20:"+now()+";"+utc);

        ZonedDateTime zdt = ZonedDateTime.ofInstant(utc, t20.UTC);        
        FIM11 fim11 = new FIM11(t20.getHour(),t20.getMinute(), t20.getSecond(), t20.getDayOfYear(), zdt.getYear(), t20.getTFOM(),0,false,false,false,0);
        System.out.println("#FIM11:"+now() + ":" + fim11.toString());
    }
    ...
    protected UTCTimeFilter bestTimeFilter=null;
    
    protected void updateUTC(int src, Instant systemTime, Instant utc, Duration age) {
        double eraw=(Duration.between(utc,systemTime).toNanos()%100_000_000_000L)/1_000_000_000.0;
        Instant utc_corrected=utc.plus(age);
        double _eraw=(Duration.between(utc_corrected,systemTime).toNanos()%100_000_000_000L)/1_000_000_000.0;
        if(latestUTC!=null) {
            if(latestUTC.compareTo(utc)<0) {
                latestUTC=utc;
                latestUTC_corrected=utc_corrected;
                latesttUTC_ST=systemTime;
                System.out.println("#T09_LATEST;"+String.format("%05d",src)+";"+utc+";"+utc_corrected+";"+age.toNanos()+";"+eraw+";"+_eraw);           
            }else{
                return;
            }
        }else{
        ...
        System.out.println("T09_DT_RAW:"+String.format("%05d",src)+";"+String.format("%.6f",_eraw)+";"+systemTime+";"+utc);
        utf.update(utc_corrected, systemTime);
        latestTimeFilter=utf;
        Double err=utf.getLastError();
        if(err!=null) {
            System.out.println("T09_ERR;"+String.format("%05d",src)+";"+String.format("%.6f",err)+";"+utf.getOffset().toMillis());
        }
    ...
    protected void updateGPSPosition(Instant systemTime, Instant utc, GINS_T09 t09) {
        ...
        System.out.println("#FIM31:"+now() + ":" + fim31.toString());
        System.out.println("#GPS;"+utc+ ";" +t09.getLatitude()+";"+t09.getLongitude()+";"+t09.getGpsAltitude()+";"+t09.getGpsFOM());
        ...
        System.out.println("#GPS_POS;"+String.format("%05d",t09.getSource())+";"+systemTime+";"+utc+";"+t09.toString());
    }
    ...
    public void update(GINSMessage gm) {
        if(gm instanceof GINS_T16) {
            ...
            onT09((GINS_T09)gm);
            System.out.println(String.format("%05d",gm.getSource())+";"+gm.toString());
