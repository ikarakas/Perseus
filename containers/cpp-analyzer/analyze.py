#!/usr/bin/env python3
"""
C/C++ Analyzer Service
Standalone service for analyzing C/C++ source code and binaries
"""
...
from analyzers.cpp_analyzer import CppSourceAnalyzer
from api.models import AnalysisOptions, AnalysisRequest, Language

logging.basicConfig(level=logging.INFO)
logger = logging.getLogger(__name__)

class CppAnalyzerService:
    """Standalone C++ analyzer service"""
    
    def __init__(self):
        self.analyzer = CppSourceAnalyzer()
    ...
    async def analyze_request(self, request_file: str):
        ...
        try:
            ...
            # Perform analysis
            logger.info(f"Starting C++ analysis for {request.location}")
            ...
            logger.info(f"Completed C++ analysis, found {len(result.components)} components")
    ...
    async def run(self):
        """Main service loop"""
        logger.info("C++ Analyzer Service starting...")
        ...
        while True:
            try:
                ...
                request_files = list(requests_dir.glob("cpp_*.json"))
